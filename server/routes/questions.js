...
router.post('/batch', adminOnly, (req, res) => {
  ...
  const transaction = db.transaction(() => {
    for (const q of questions) {
      insert.run(q.type, scope || 'public', scope === 'project' ? (project_id || null) : null,
        q.category || '', q.difficulty || 1, q.content,
        JSON.stringify(q.options || null), JSON.stringify(q.answer || null),
...
router.delete('/:id', adminOnly, (req, res) => {
  const db = getDb();
  db.prepare("UPDATE questions SET status = 'deleted' WHERE id = ?").run(req.params.id);
  res.json({ success: true });
});
...
router.get('/stats', adminOnly, (req, res) => {
  const db = getDb();
  const questions = db.prepare("SELECT * FROM questions WHERE status != 'deleted'").all();
  const stats = {
