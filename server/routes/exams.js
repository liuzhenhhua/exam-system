...
router.get('/', (req, res) => {
  ...
  const exams = db.prepare(sql).all(...params).map(e => {
...
router.get('/:id', (req, res) => {
  ...
  // 附上题目快照
  const questions = db.prepare('SELECT * FROM exam_questions WHERE exam_id = ? ORDER BY sort_order').all(exam.id);
...
router.post('/', adminOnly, (req, res) => {
  ...
  if (exam.status === 'published' && exam.rules && exam.rules.length > 0) {
    generateSnapshot(db, examId, exam.rules, exam.scope, exam.project_id);
  }
...
router.put('/:id', adminOnly, (req, res) => {
  ...
  if (newStatus === 'published' && newRules.length > 0) {
    generateSnapshot(db, id, newRules, exam.scope, exam.project_id);
  }
...
// 内部：生成题目快照
function generateSnapshot(db, examId, rules, scope, projectId) {
  let pool;
  if (scope === 'project' && projectId) {
    pool = db.prepare("SELECT * FROM questions WHERE status != 'deleted' AND scope = 'project' AND project_id = ?").all(projectId);
  } else {
    pool = db.prepare("SELECT * FROM questions WHERE status != 'deleted' AND (scope = 'public' OR scope IS NULL)").all();
  }
  ...
  for (const rule of rules) {
    ...
    selected.forEach(q => {
      insert.run(examId, q.id, q.type, q.content, q.options, q.answer, q.analysis, q.difficulty, q.category, rule.score || 10, sortOrder++);
  ...
  if (insertedIds.length > 0) {
    db.prepare('UPDATE exams SET question_count = ? WHERE id = ?').run(insertedIds.length, examId);
  }
}
